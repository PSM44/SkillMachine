        ...
        if ($env:SKILLS_DEBUG_UC04 -eq "1") {
            try {
                if ($UseCaseName -eq "04.REPOSITORY_STRUCTURE_REPAIR") {
                    ...
                    Write-Host ("DEBUG[UC04]: DeliveryFilesExtra.Count = {0}" -f @($UC_DeliveryFilesExtra).Count)
                    if (@($UC_DeliveryFilesExtra).Count -gt 0) {
                        $sample = @($UC_DeliveryFilesExtra)[0..([Math]::Min(4,@($UC_DeliveryFilesExtra).Count-1))]
