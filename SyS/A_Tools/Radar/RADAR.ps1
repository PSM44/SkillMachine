...
$ExcludedPaths = @()
$ExcludedSeen = @{}
foreach ($ex in $ExcludedPathsRaw) {
    ...
    if (-not $ExcludedSeen.ContainsKey($key)) {
        $ExcludedSeen[$key] = $true
        $ExcludedPaths += $resolved
    }
}
...
function Test-IsExcluded {
    ...
    foreach ($Excluded in $ExcludedPaths) {
        ...
        try {
            $ExcludedFull = [System.IO.Path]::GetFullPath($ExcludedFull)
        } catch {
            $ExcludedFull = $Excluded
        }

        $ExcludedFull = $ExcludedFull.TrimEnd('\', '/')

        if ($Candidate.Equals($ExcludedFull, [System.StringComparison]::OrdinalIgnoreCase)) {
            return $true
        }

        if ($Candidate.StartsWith($ExcludedFull + "\", [System.StringComparison]::OrdinalIgnoreCase)) {
            return $true
        }
